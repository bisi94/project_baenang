			...
			if (responseCode == 200) {
				...
				if ("0000".equals(responseHeader.get("resultCode").getAsString())) {
					...
					if (items.size() > 0) {
						...
						System.out.println("items.size() is lower than 1");
					}
				} else {
					System.out.println("resultcode isn't '0000'");
				}
			} else {
				System.out.println("response code isn't 200! check plz " + responseCode);
			}
		}catch(Exception e){
			e.printStackTrace();
		}
